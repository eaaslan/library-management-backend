...
public class DummyDataInitializer implements CommandLineRunner {
    ...
    public void run(String... args) {
        ...
        if (bookRepository.count() == 0) {
            ...
            bookIds = bookRepository.findAll().stream().map(Book::getId).toList();
        }

        if (borrowingRepository.count() == 0) {
            createBorrowingsWithNewTransaction(bookIds);
        } else {
            log.info("Borrowings already exist. Skipping borrowing creation.");
        }
    ...
    public void createUsers() {
        ...
        // Fourth Active Patron
        if (!userRepository.existsByEmail("patron4@library.com")) {
            User patron4 = User.builder()
                    .email("patron4@library.com")
                    .password(passwordEncoder.encode("patron123"))
                    .firstName("Third")
                    .lastName("Patron")
                    .phoneNumber("05959939963")
                    .role(UserRole.PATRON)
                    .status(UserStatus.ACTIVE)
                    .maxAllowedBorrows(3)
                    .build();
            userRepository.save(patron4);
            log.info("Third active patron created: {}", patron4.getEmail());
        }
    ...
    Long getRandomAvailableBookId(List<Long> bookIds) {
